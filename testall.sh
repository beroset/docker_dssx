...
build_one() (
    ...
    $(time ${CONTAINER_ENGINE} build ${BUILD_OPTS}  -f=work/Dockerfile.$distro -t beroset/opendssx/$distro work >>"${LOGFILE}" 2>&1 )
)
...
test_one() (
    ...
    ${CONTAINER_ENGINE} run --rm -v "${SHARED_DIR}":/mnt/host:z "beroset/opendssx/${distro}" "compile /mnt/host/StevensonPflow-3ph.dss" 1>>${LOGFILE} 2>&1
